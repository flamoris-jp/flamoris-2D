...
public partial class MainWindow
{
    ...
    private void ConfigureRigContext()
    {
        ...
        if(_editingContext==EditingContext.Rig)
        {
            ...
            ActiveContextBadge.Text=$"リグ / {AuthoringLabels.Rig(_rigContext)}";
            ActiveToolSettingsText.Text=$"{_rigTool} — {AuthoringLabels.Rig(_rigContext)}の編集";
        }
    ...
    private void BuildBonePanel(RigContext context,long revision)
    {
        ...
        var select=Choices(panel,"編集する骨（Bone）",BoneChoices(),_selectedBoneId);
        ...
        ActionButton(panel,"新しい骨／選択した骨の子を追加",()=>RunRigEditAsync(()=>RigEdit.CreateBone(name.Text,ReadNumber(length)),context,revision));
        if(context.BoneId is not null)
        {
            ...
            var boneEnabled=Check(panel,"骨を有効にする",Property(bone,"enabled").ValueKind!=JsonValueKind.False);
            ActionButton(panel,"骨の有効状態を確定",()=>RunRigEditAsync(()=>RigEdit.EnableBone(boneEnabled.IsChecked==true),context,revision));
            ActionButton(panel,"ポーズをリセット",()=>RunRigEditAsync(RigEdit.ResetBone,context,revision));
            ActionButton(panel,"選択パーツをこの骨に固定",()=>RunRigEditAsync(RigEdit.BindBone,context,revision));
            ...
            {
                var rigidEnabled=Check(panel,"骨への固定を有効にする",Property(rigid,"enabled").ValueKind==JsonValueKind.True);
            ...
            var parent=Choices(panel,"親の骨／ルート",parents,String(bone,"parentNodeId"));
            ...
            var mirror=Choices(panel,"左右反転先の骨",BoneChoices().Where(b=>b.Id!=context.BoneId));var axis=Field(panel,"反転軸 X",0);
            ...
            ActionButton(panel,"骨を削除",()=>RunRigEditAsync(RigEdit.RemoveBone,context,revision));
        }
        var mid=Choices(panel,"2ボーンIK 中間の骨",BoneChoices());var end=Choices(panel,"2ボーンIK 終端の骨",BoneChoices());
        var clockwise=Check(panel,"時計回りに曲げる",false);
        ActionButton(panel,"選択した骨を根にしてIKを作る",()=>RunRigEditAsync(()=>RigEdit.CreateIk(context.BoneId??throw new ArgumentException("根となる骨を選択してください。"),Chosen(mid),Chosen(end),clockwise.IsChecked==true),context,revision));
    ...
    private void BuildWarpPanel(RigContext context,long revision)
    {
        ...
        var select=Choices(panel,"編集する格子（Warp）",warps.Select(w=>new EntityChoice(String(w,"id")!,String(w,"displayName")??"Warp")),context.DeformerId);
        ...
        ActionButton(panel,"格子を作成して選択パーツを入れる",()=>RunRigEditAsync(()=>RigEdit.CreateWarp(name.Text,Chosen(parent),int.Parse(Chosen(grid)),ReadNumber(left),ReadNumber(top),ReadNumber(right),ReadNumber(bottom),context.NodeId is { } n?[n]:[]),context,revision));
        ...
        ActionButton(panel,"選択対象をこの格子へ移動",()=>RunRigEditAsync(()=>RigEdit.AttachWarp(context.DeformerId),context,revision));
        ActionButton(panel,"この格子の親を変更",()=>RunRigEditAsync(()=>RigEdit.AttachWarp(Chosen(parent)),context with {NodeId=context.DeformerId},revision));
        ...
        ActionButton(panel,"格子を削除（中の対象は保持）",()=>RunRigEditAsync(RigEdit.RemoveWarp,context,revision));
    }
    private void BuildWeightPanel(RigContext context,long revision)
    {
        var panel=AuthoringPanel;var bone=Choices(panel,"ウェイトを編集する骨（Bone）",BoneChoices(),context.BoneId);
        ...
        Note(panel,"パーツと骨を選び、頂点を塗って影響度を調整します。ひと塗りを1回で元に戻せます。");
        ActionButton(panel,"全頂点をこの骨へ割り当てて開始",()=>RunRigEditAsync(()=>RigEdit.CreateSkin(String(topology,"id")??throw new ArgumentException("先にメッシュを作成してください。")),context,revision));
        if(context.BindingId is not null)
        {
            ...
            var replace=Section(panel,"影響する骨を明示して置換");
            Note(replace,"頂点を1つ選択し、最大4本の骨とウェイトを指定します。合計は1にしてください。空欄は使いません。");
            var influences=new List<(ComboBox Bone,TextBox Weight)>();
            for(var i=0;i<4;i++)influences.Add((Choices(replace,$"骨 {i+1}",BoneChoices().Prepend(new EntityChoice("","—")),i==0?context.BoneId:""),Field(replace,$"ウェイト {i+1}",i==0?1:0)));
            ...
            ActionButton(replace,"選択頂点の全ウェイトを解除（スキニング無効時）",()=>RunRigEditAsync(()=>RigEdit.ClearWeights(MeshCanvas.Selected.Single()),context,revision));
            ...
            ActionButton(panel,"骨とメッシュの関連付けを削除",()=>RunRigEditAsync(RigEdit.RemoveSkin,context,revision));
        }
    ...
    {
        Note(AuthoringPanel,"形状補正：骨や格子で動かした後の形を整えます。メッシュの構造・原画上の位置決めは変更しません。");
