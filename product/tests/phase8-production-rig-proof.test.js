...
test("BoneTrack mixes over the KeyArt pose and one rotation constraint runs before FK/rigid binding", () => {
  ...
  const constrained = evaluateSequence(project, "sequence_proof", 60);
  const unconstrainedProject = structuredClone(project);
  unconstrainedProject.rig.boneRotationConstraints = [];
  const unconstrained = evaluateSequence(unconstrainedProject, "sequence_proof", 60);
  ...
  const exactClamped = evaluateSequence(exactClampedProject, "sequence_proof", 60);
...
test("DeformerTrack overlays nested Warp parent-first and one cage drives mesh and Bone projection", () => {
  const { project } = buildPhase8ProductionProof();
  const animated = evaluateSequence(project, "sequence_proof", 50);
  ...
  const noHair = evaluateSequence(noHairProject, "sequence_proof", 50);
  ...
  assert.deepEqual(evaluateSequence(reversed, "sequence_proof", 50), animated);
});
...
test("MeshFormCorrection remains before ordinary MeshDeformationTrack contribution", () => {
  ...
  const fullPosition = partMesh(evaluateSequence(project, "sequence_proof", 60), "slot_body");
  const noDetailPosition = partMesh(evaluateSequence(noDetail, "sequence_proof", 60), "slot_body");
  const noFormPosition = partMesh(evaluateSequence(noForm, "sequence_proof", 60), "slot_body");
  const neitherPosition = partMesh(evaluateSequence(neither, "sequence_proof", 60), "slot_body");
...
test("clipping resolves the final deformed mask instance while Blink and Warp are active", () => {
  ...
  const frame = session.query("sequence.evaluate", { sequenceId: "sequence_proof", timeTicks: 150 });
...
test("production evaluation never mutates KeyArt, Bone, Warp, or Form keyforms", () => {
  ...
  for (const timeTicks of [0, 50, 100, 150, 200, 250, 300, 350, 400, 450, 600]) {
