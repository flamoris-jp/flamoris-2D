...
| Live desktop MCP discovery/authentication/multi-client attachment | mcp/adapter.js; ADR 0006 | existing shared-session adapter | all | Session | not publicly exposed | intentionally deferred | product-host.test.js shared-session parity | G5: accepted design requires a separate transport/authentication ADR before exposing a live desktop. Not replaced by a second editor. |
